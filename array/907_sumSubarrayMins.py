...
class Solution(object):
    ...
    # TODO
    # solution 2 還沒懂
    # def sumSubarrayMins_discussion_solution_2_right(self, A):
    #     """
    #     :type A: List[int]
    #     :rtype: int
    #     """
    ...
    def sumSubarrayMins_discussion_solution_1_right(self, A):
        """
        :type A: List[int]
        :rtype: int
        """
        left = [(a, 1) for a in A]
        right = [(a, 1) for a in A]
        stack = []
        ...
        for i in range(len(A)-1, -1, -1):
            ...
            # print(A[i], right)
            right[i] = (A[i], count)
            stack.append((A[i], right[i][1]))
        total = 0
        # print ("left=", left)
        # print ("right=", right)
        for i in range(len(A)):
            total = (total + left[i][1] * right[i][1] * A[i]) % (10 ** 9 + 7)
        return total

    def sumSubarrayMins_discussion_solution_1_wrong2(self, A):
        """
        :type A: List[int]
        :rtype: int
        """
        ...
        for i in range(len(A)):
            # BUG!!! fix in right
            total += left[i][1]*right[i][1]*A[i] % (10 ** 9 + 7)
        return total

    def sumSubarrayMins_discussion_solution_1_wrong_1(self, A):
        """
        :type A: List[int]
        :rtype: int
        """
        ...
        for i in range(len(A)-1, -1, -1):
            count = 1
            # BUG!!!! the same num doesn't need to be count twice
            while stack and stack[-1][0] > A[i]:
                count += stack[-1][1]
                stack.pop()
            # print(A[i], right)
    ...
    def sumSubarrayMins_self_solution_dp_TLE(self, A):
        """
        :type A: List[int]
        :rtype: int
        """
